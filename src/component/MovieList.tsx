...
const MovieList = ({ searchTerm }: { searchTerm: string }) => {
  ...
  const allMovies = useMemo(() => {
    ...
    const uniqueMovies = Array.from(new Set(all?.map((movie) => movie.id))).map(
      (id) => all?.find((movie) => movie.id === id)
    );
  ...
  return (
    <div className="max-w-[1200px] mx-auto p-5">
      ...
      {allMovies.length == 0 && (
        <div className="w-full flex justify-center text-sm text-red-400">
